...
function loadConfig() {
  ...
  let saved = {};
  try { saved = JSON.parse(localStorage.getItem('tt_config_v2') || '{}'); } catch (_) {}
...
function saveSettings() {
  ...
  localStorage.setItem('tt_config_v2', JSON.stringify(state.config));
...
function applyDefaults() {
  localStorage.removeItem('tt_config_v2');
  loadConfig();
  render();
  showToast('已恢复默认');
}
